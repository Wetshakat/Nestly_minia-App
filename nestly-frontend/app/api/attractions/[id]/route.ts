...
export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    if (MOCK_ATTRACTION_DETAILS[attractionId]) {
      console.log("Returning mock attraction details for ID:", attractionId)
    ...
    if (!attractionResponse.ok) {
      console.warn(" Attraction not found in Django, returning mock data")
    ...
    console.log("Returning mock attraction as fallback")
    return NextResponse.json(MOCK_ATTRACTION_DETAILS["1"])
  }
}
